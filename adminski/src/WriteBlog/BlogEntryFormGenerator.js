...
class BlogEntryFormGenerator extends React.Component {
    ...
    sectionDeletedCallback = (idx) => {
        ...
        this.state.blogSectionsOnscreen.map((section, index)=>{
            if(index !== idx){
                ...
                //push null onto the array, not sure why, but it doesnt work if we dont change
                // deleted index of array to null
                filteredBlogEntrySectionArr.push(null);
            }
        })
    ...
    createBlogDataModel(){
        console.log('creating model ', this.state.blogSectionsOnscreen);
        let blogTextData = [];
        this.state.blogSectionsOnscreen.forEach((nextBlogSection) => {
            //check for null
            if(nextBlogSection === null){
                return;
            }
